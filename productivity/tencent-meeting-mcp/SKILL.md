...
**运行环境**：依赖 Python 3，首次使用时自动检测可用的 Python 命令：
- macOS / Linux：通常为 `python3`
- Windows：通常为 `python`（Python 3 安装后默认注册为 `python`）

首次使用时执行 `python --version` 或 `python3 --version` 检查，优先使用能成功执行的命令。

**Token 配置**：Token 由本地凭证代理自动获取，无需手动配置。若自动获取失败，可手动访问 https://meeting.tencent.com/ai-skill 获取 Token，配置环境变量 `TENCENT_MEETING_TOKEN`。

---

## ⚠️ 调用方式（必读）

所有工具操作**必须**通过执行 Python 脚本完成，**禁止直接调用工具名**。

**命令格式**：
```bash
# macOS / Linux
python3 scripts/tencent_meeting.py <method> [params_json]

# Windows（使用 python 而非 python3，且必须先设置 UTF-8 编码）
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; python scripts/tencent_meeting.py <method> [params_json]
```

> ⚠️ **跨平台规则**：先尝试 `python3`，若提示命令不存在则改用 `python`。Windows 系统上通常只有 `python` 命令。
> 
> ⚠️ **Windows 编码**：Windows PowerShell 默认编码非 UTF-8，会导致中文乱码。**每次调用前必须**执行 `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8` 设置编码，可与 python 命令写在同一行用 `;` 分隔。

**快速示例**：
```bash
# 列出所有可用工具
python3 scripts/tencent_meeting.py tools/list    # macOS/Linux
python scripts/tencent_meeting.py tools/list     # Windows

# 调用具体工具（查询会议）
python3 scripts/tencent_meeting.py tools/call '{"name": "get_user_meetings", "arguments": {"pos": 0, "cursory": 20, "is_show_all_sub_meetings": 0, "_client_info": {"os": "auto", "agent": "qclaw", "model": "auto"}}}'
```
